...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--require-structure-map-renderer",
        action="store_true",
        help="Require structure-map.html to be generated by structure-map-html/scripts/render-structure-map.mjs.",
    )
    return parser
...
def validate_structure_map(
    path: Path,
    require_local_cytoscape: bool,
    require_structure_map_renderer: bool,
) -> list[str]:
    errors: list[str] = []
    if not path.exists():
        return [f"missing file: {path}"]
    ...
    if require_structure_map_renderer:
        if 'data-generated-by="structure-map-renderer-v1"' not in lower:
            errors.append("structure-map.html was not generated by structure-map-html/scripts/render-structure-map.mjs")
        ...
        if file_name == "structure-map.html":
            errors.extend(
                validate_structure_map(
                    path,
                    args.require_local_cytoscape,
                    args.require_structure_map_renderer,
                )
            )
