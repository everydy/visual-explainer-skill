...
새 `structure-map.html`을 만들 때는 HTML을 처음부터 새로 쓰지 않는다. 기본은 아래 순서다.

1. `templates/structure-map-cytoscape.html`을 먼저 읽는다.
2. 그 파일을 출력 위치의 `structure-map.html`로 복사하거나 같은 구조로 시작한다.
3. 제목, `?` 도움말 copy, 하단 한 줄 legend, fallback 문구, 오른쪽 설명 copy, `graphElements`만 입력 주제에 맞게 교체한다.
4. 그래프 렌더링, 관계 라벨 overlay, 선택/해제, 오른쪽 설명 패널, fallback, 하단 legend, 전체 보기 조작은 유지한다.
5. 템플릿 장치를 제거하거나 다른 layout을 써야 할 만큼 특수한 구조라면, `structure-map.html` 안 또는 최종 답변에 이유를 짧게 남긴다.

## 자유도 원칙

`skill-creator` 기준으로 이 스킬은 중간 자유도를 둔다.

- 낮은 자유도: 관계 라벨이 읽히는 것, 노드/엣지 클릭 설명이 되는 것, 선택한 항목과 나머지 문맥이 구분되는 것, fallback이 있는 것은 깨면 안 된다.
- 중간 자유도: 템플릿의 기본 함수 이름, overlay 구조, 카운트/legend/설명 패널은 유지하되 입력 주제에 맞춰 데이터와 좌표를 바꾼다.
- 높은 자유도: 노드 배치, 색상 의미, relation copy, 보조 layout, 노드 수가 많을 때의 군집화 방식은 주제에 맞게 판단한다.
